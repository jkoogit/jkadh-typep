...
### 🚨 무태스크 소스/문서 수정 엄격 제한 (Strict No-Task Source & Docs Modification Gate)
- **단위작업 소스 및 문서수정 제한**: 명시적인 단위 기능 개발, 버그 수정, 리팩토링 및 설계/가이드/명세 문서 작업 시 `#태스크시작`을 거치지 않고 소스코드 및 문서(`src/*`, `scripts/*`, `docs/*` 등)를 직접 수정/생성하는 행위를 원천 금지합니다.
- **일괄/루프 작업 프롬프트 거버넌스**: 사용자가 "태스크 시작-처리-정리-승급 일괄 진행" 또는 "루프 실행"을 단일 프롬프트로 요청한 경우에도, 에이전트는 하네스 각 단계(이슈 생성 ➔ 7-Phase 검증 ➔ PR 생성/머지 ➔ 승급)를 투명하게 실행하고 각 단계별 상태 및 표준 프롬프트를 명시적으로 보고해야 합니다.
...
## 🛡️ Git 저장소 보호 및 파일 처리 불변 거버넌스 (Git Integrity Guard)

### 1. [사전 예방 제약] Git 메타데이터 보호 및 파일 처리 불변 원칙 (Strict Prevention Hard Gate)
- **.git 디렉터리 보호 (절대 삭제 금지)**: 어떠한 경우에도 `/.git/` 하위의 메타데이터 파일(HEAD, config, refs, objects, hooks, logs 등)을 `delete_file`, `delete_dir` 등의 툴로 삭제하거나 임의 변조하는 행위를 엄격히 금지합니다.
- **파괴적 명령어 실행 금지**: 로컬 소스코드나 추적 정보를 파괴할 수 있는 `git clean -fd`, `git reset --hard HEAD~`, `rm -rf .git` 등의 파괴적 셸 명령을 실행할 수 없습니다.
- **작업 파일 안전 보존**: 파일 삭제 및 정리 시에는 `src/`, `docs/` 내의 명시적 대상 파일만 조작하며, 추적되지 않은 파일(Untracked files)이 있더라도 사용자의 명시적 요청 없이 일괄 삭제하지 않습니다.

### 2. [부가 조치 기능] 이상 감지 시 비상 자가 복구 파이프라인 (Auto-Healing Fallback)
- **발동 조건**: 플랫폼 샌드박스 동기화 글리치 등으로 인해 `fatal: not a git repository` 오류가 감지된 경우에 한해 즉시 부가 복구 조치를 실행합니다.
- **표준 복구 절차 (무손실 보존)**:
  1. `git init -b dev` (기본 브랜치 초기화)
  2. `git config user.name "jkoogit" && git config user.email "jkoogit@gmail.com"`
  3. `git remote add origin https://github.com/jkoogit/jkadh-typep.git`
  4. `git fetch origin`
  5. `git reset origin/dev` (작업 중인 미커밋 소스코드를 100% 보존하며 Git 히스토리만 원격과 동기화)
